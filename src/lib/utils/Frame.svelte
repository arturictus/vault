<script lang="ts">
  ...
  // Compute the tag based on presence of href
  const elementTag = $derived(href ? "a" : tag || "div");

  // Using derived state for color with fallback
  const currentColor = $derived(color ?? "default");

  // Update color context when color changes
  $effect(() => {
    setContext("color", currentColor);
  });
  ...
  $effect(() => {
    ...
    // Dispatch show event
    const showEvent = new CustomEvent("show", { detail: open });
    document.dispatchEvent(showEvent);
  });

  // Using derived state for the class
  const divClass = $derived(
    twMerge(
      bgColors[currentColor],
      textColors[currentColor],
      rounded && "rounded-lg",
      border && "border",
      borderColors[currentColor],
      shadow && "shadow-md",
      className,
    ),
  );
