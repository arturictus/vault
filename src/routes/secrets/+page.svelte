<script>
  ...
  import paginationData from '../../lib/utils/advancedTable.json';
  ...
  $: filteredItems = paginationData.filter((item) => item.product_name.toLowerCase().includes(searchTerm.toLowerCase()));
</script>

<Section name="advancedTable" classSection="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
  <TableSearch placeholder="Search" hoverable bind:inputValue={searchTerm} class={divClass} innerClass={innerDivClass} searchClass={searchClass} inputClass={classInput}>
