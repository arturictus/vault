...
impl TryFrom<piv_card::AlgorithmId> for EccAlgorithm {
    type Error = &'static str;
    
    fn try_from(value: piv_card::AlgorithmId) -> Result<Self, Self::Error> {
        match value {
...
impl PublicKey {
    ...
    pub fn encrypt(&self, data: &[u8]) -> Result<Vec<u8>, &'static str> {
        encrypt(data, &self.bytes, self.algorithm)
    }

    pub fn verify(&self, data: &[u8], signature_bytes: &[u8]) -> Result<bool, &'static str> {
        verify(data, signature_bytes, &self.bytes, self.algorithm)
    }
}
...
pub fn encrypt(
    ...
    match algorithm {
        EccAlgorithm::P256 => {
            ...
            let mut okm = [0u8; 44];
            hk.expand(b"aes-256-gcm-key-nonce", &mut okm)
...
pub fn decrypt(
    ...
    match algorithm {
        EccAlgorithm::P256 => {
            ...
            let (ephemeral_public_key_bytes, ciphertext) =
                encrypted_data_with_key.split_at(ephemeral_pk_size);
            ...
            let mut okm = [0u8; 44];
            hk.expand(b"aes-256-gcm-key-nonce", &mut okm)
                .map_err(|_| "HKDF expansion failed for P256 decryption")?;
            let (key_bytes, nonce_bytes) = okm.split_at(32);
        ...
        EccAlgorithm::P384 => {
            ...
            let mut okm = [0u8; 44];
            hk.expand(b"aes-256-gcm-key-nonce", &mut okm)
                .map_err(|_| "HKDF expansion failed for P384 decryption")?;
            let (key_bytes, nonce_bytes) = okm.split_at(32);
...
pub fn from_piv_algorithm_id(
    piv_alg_id: piv_card::AlgorithmId,
) -> Result<EccAlgorithm, &'static str> {
    match piv_alg_id {
        piv_card::AlgorithmId::EccP256 => Ok(EccAlgorithm::P256),
        piv_card::AlgorithmId::EccP384 => Ok(EccAlgorithm::P384),
        _ => Err("Unsupported PIV AlgorithmId for ECC"),
...
mod tests {
    ...
    fn test_sign_verify_p256() {
        ...
        let is_valid = verify(data, &signature, &public_key, EccAlgorithm::P256)
            .expect("P256 verification failed");
        assert!(is_valid);
    }
    ...
    fn test_sign_verify_p384() {
        ...
        let is_valid = verify(data, &signature, &public_key, EccAlgorithm::P384)
            .expect("P384 verification failed");
        assert!(is_valid);
    }
    ...
    fn test_sign_verify_p256_invalid_signature() {
        ...
        let is_valid = verify(wrong_data, &signature, &public_key, EccAlgorithm::P256)
            .expect("P256 verification with wrong data failed");
        assert!(!is_valid);

        let mut tampered_signature = signature.clone();
        tampered_signature[0] ^= 0xff; // Tamper with the signature
        let is_valid_tampered = verify(data, &tampered_signature, &public_key, EccAlgorithm::P256)
            .expect("P256 verification with tampered signature failed");
    ...
    fn test_encrypt_decrypt_p256_wrong_key() {
        ...
        let (wrong_private_key_bytes, _wrong_public_key_bytes) =
            generate_key_pair(EccAlgorithm::P256);
        let original_data = b"another secret message";

        let encrypted_data = encrypt(original_data, &public_key_bytes, EccAlgorithm::P256)
            .expect("P256 encryption failed");

        let decryption_result = decrypt(
            &encrypted_data,
        ...
        assert!(
            ...
            "Decryption with wrong P256 key should fail"
        );
        assert_eq!(
            decryption_result.unwrap_err(),
            "AES-GCM P256 decryption failed"
        );
    }
    ...
    fn test_encrypt_decrypt_p384_wrong_key() {
        ...
        let (wrong_private_key_bytes, _wrong_public_key_bytes) =
            generate_key_pair(EccAlgorithm::P384);
        ...
        assert!(
            decryption_result.is_err(),
            "Decryption with wrong P384 key should fail"
        );
        assert_eq!(
            decryption_result.unwrap_err(),
            "AES-GCM P384 decryption failed"
        );
    }
    ...
    fn test_from_piv_algorithm_id() {
        assert!(matches!(
            from_piv_algorithm_id(PivAlgorithmId::EccP256),
            Ok(EccAlgorithm::P256)
        ));
        assert!(matches!(
            from_piv_algorithm_id(PivAlgorithmId::EccP384),
            Ok(EccAlgorithm::P384)
        ));
